"""Taken from ModelDiff's Supplementary material"""
...
class SDog120(data.Dataset):
    def __init__(
        self, root, is_train=True, transform=None, shots=5, seed=0, preload=False
    ):
        ...
        if is_train:
            ...
            if shots > 0:
                ...
                for c in range(self.num_classes):
                    ...
                    random.seed(seed)
                    random.shuffle(ids)  # type: ignore
                    count = 0
                    for i in ids:
    ...
    def __getitem__(self, index):
        if self.preload:
            ...
            img = Image.open(self.image_path[index]).convert("RGB")  # type: ignore
