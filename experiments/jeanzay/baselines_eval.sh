#!/bin/bash
#SBATCH --job-name=BaselinesEval            # Job name
...
for budget in 10 20 50 100 200 400; do
    for SAMPLER in "Random" "Negative" "NegativeAugment" "Adversarial" "NegativeAdversarial" "Boundary" "RandomSubsample10" "RandomSubsample100" "NegativeSubsample10" "NegativeSubsample100"; do  
        ...
        # Sample the queries
        srun python main.py ${config} sample --benchmark=${benchmark} --dataset=${dataset} --split=${split} --budget=${budget} --sampler=$SAMPLER

        # Baseline distance on hard labels
        srun python main.py ${config} generate --benchmark=${benchmark} --dataset=${dataset} --split=${split} --budget=${budget} --sampler=$SAMPLER --representation="Labels"
        srun python main.py ${config} pair-distance --benchmark=${benchmark} --dataset=${dataset} --split=${split} --budget=${budget} --sampler=$SAMPLER --representation="Labels" --distance="hamming"

        # Baseline distance on soft labels
        srun python main.py ${config} generate --benchmark=${benchmark} --dataset=${dataset} --split=${split} --budget=${budget} --sampler=$SAMPLER --representation="Logits"
        srun python main.py ${config} pair-distance --benchmark=${benchmark} --dataset=${dataset} --split=${split} --budget=${budget} --sampler=$SAMPLER --representation="Logits" --distance="cosine"
    done
done
