...
class GPSA(nn.Module):
    ...
    def get_attention(self, x):
        ...
        q, k = qk[0], qk[1]
        pos_score = self.rel_indices.expand(B, -1, -1, -1).to(dtype=x.dtype)
...
def convit_tiny(pretrained=False, **kwargs) -> ConVit:
    ...
    model = _create_convit(
        variant="convit_tiny", pretrained=pretrained, **dict(model_args, **kwargs)
    )
    return model
...
def convit_small(pretrained=False, **kwargs) -> ConVit:
    ...
    model = _create_convit(
        variant="convit_small", pretrained=pretrained, **dict(model_args, **kwargs)
    )
    return model
...
def convit_base(pretrained=False, **kwargs) -> ConVit:
    ...
    model = _create_convit(
        variant="convit_base", pretrained=pretrained, **dict(model_args, **kwargs)
    )
    return model
